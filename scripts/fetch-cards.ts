#!/usr/bin/env bun
/**
 * Seed script — populates src/data/cards.generated.ts from the Pokémon TCG API.
 *
 ...
 *
 *   bun run scripts/fetch-cards.ts      (or: npm run update-cards)
 *
...
import { setCodeMap, normalizeEnergyCost, normalizeCardNumber } from '../src/services/pokemonTcgApi.ts';
...
async function main(): Promise<void> {
  ...
  const out =
    ...
    `// Refresh with: npm run update-cards (bun run scripts/fetch-cards.ts)\n` +
    `import type { CardData } from './decks';\n\n` +
    `export const generatedCards: Record<string, CardData> = {\n${entries}\n};\n\n` +
    `export default generatedCards;\n`;

  const outPath = new URL('../src/data/cards.generated.ts', import.meta.url);
  writeFileSync(outPath, out);
  console.log(`\nWrote ${count} Pokémon cards to src/data/cards.generated.ts`);
}
